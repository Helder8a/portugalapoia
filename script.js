...
document.addEventListener("DOMContentLoaded", function() {
    // Asumiendo que `loadJobs` es la función existente en tu script
    const loadBlogPosts = async () => {
        ...
        const blogClearButton = document.getElementById('blogClearButton');
        ...
        try {
            ...
            const data = await response.json();
            allBlogPosts = data.artigos;
        } catch (error) {
            console.error("Erro ao carregar os artigos do blog:", error);
            blogPostsGrid.innerHTML = '<p class="text-danger">Não foi possível carregar os artigos do blog. Por favor, tente novamente mais tarde.</p>';
            return;
        }
                ...
                posts.forEach(post => {
                    const postHtml = `
                        <div class="col-md-6 col-lg-4 mb-4">
                            <div class="card blog-post h-100 shadow-sm">
                                <img src="${post.imagem || 'https://via.placeholder.com/400x200.png?text=Imagem+em+breve'}" class="card-img-top" alt="${post.titulo}" onerror="this.src='https://via.placeholder.com/400x200.png?text=Imagem+em+breve'">
                                <div class="card-body d-flex flex-column">
                                    <h5 class="card-title">${post.titulo}</h5>
                                    <p class="card-text text-muted">${post.extracto}</p>
                                    <div class="d-flex justify-content-between align-items-center mt-auto">
                                        <span class="badge badge-primary">${post.categoria}</span>
                                        <small class="text-muted">Publicado em ${post.data}</small>
                                    </div>
                                </div>
                                <div class="card-footer bg-white border-top-0">
                                    <a href="post.html?id=${post.id}" class="btn btn-primary btn-block">Ler mais</a>
                                </div>
                            </div>
                        </div>
                    `;
                    blogPostsGrid.innerHTML += postHtml;
                });
            }
        };
        ...
        const filterBlogPosts = () => {
            ...
            const selectedCategory = activeButton ? activeButton.dataset.category : 'todos';

            const filteredPosts = allBlogPosts.filter(post => {
                const matchesCategory = selectedCategory === 'todos' || post.categoria.toLowerCase() === selectedCategory;
                const matchesQuery = post.titulo.toLowerCase().includes(query) ||
                                     post.extracto.toLowerCase().includes(query) ||
        ...
        blogClearButton.addEventListener('click', () => {
