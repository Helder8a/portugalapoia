document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function loadAndFilterBlogPosts() {
        ...
        if (!grid) return; // Solo ejecutar en la página del blog
        ...
        try {
            ...
            filterButtons.forEach(button => {
                button.addEventListener('click', () => {
                    ...
                    const filteredPosts = (category === 'all') ? allPosts : allPosts.filter(post => post.categoria === category);
            ...
            grid.innerHTML = '<p class="col-12 text-center text-danger">Não foi possível carregar as publicações.</p>';
        }
    }
    ...
    function displayBlogPosts(posts) {
        ...
        posts.forEach(post => {
            ...
            postCol.className = 'col-lg-4 col-md-6 mb-4';
            const bodyHtml = post.corpo ? post.corpo.replace(/\n/g, '<br>').substring(0, 120) + '...' : '';
            const postImage = post.imagem ? `<img src="${post.imagem}" class="card-img-top" alt="${post.titulo}">` : '';
            ...
            postCol.innerHTML = `
                <div class="card blog-card shadow-sm h-100">
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <p class="card-text flex-grow-1">${bodyHtml}</p>
