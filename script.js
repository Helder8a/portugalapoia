document.addEventListener("DOMContentLoaded", async () => {
  ...
  t &&
    (window.addEventListener("load", () => t.classList.add("hidden")),
  ...
  if (a) {
    const t = document.body,
      e = (e) => {
        "dark" === e
          ? (t.classList.add("dark-theme"), (a.checked = !0))
          : (t.classList.remove("dark-theme"), (a.checked = !1));
      },
  ...
  async function o(t, e, a, o) {
    ...
    if (!c || 0 === c.length)
      return void (
        "gallery-section" !== e &&
        (s.innerHTML =
          '<p class="col-12 text-center lead text-muted mt-5">De momento, não há publicações nesta secção.</p>')
      );
    c.sort(
      (t, e) =>
        new Date(e.data_publicacao || e.date || 0) -
        new Date(t.data_publicacao || t.date || 0)
    );
  ...
  function s(t) {
    if (!t || !t.data_publicacao || !t.data_vencimento)
      return `<div class="date-info">ID: ${t.id || "N/A"}</div>`;
    ...
    const o = e.toLocaleDateString("pt-PT", {
      ...
      c = a < n;
    return `<div class="date-info">Publicado: ${o} <br> <span class="${
      c ? "vencido" : ""
    }">Vencimento: ${s} ${c ? "(Vencido)" : ""}</span></div>`;
  }
  ...
  function c(t, e) {
    ...
    return `<div class="share-buttons"><small class="share-label">Partilhar:</small><a href="https://api.whatsapp.com/send?text=${encodeURIComponent(
      n
    )}%20${o}" target="_blank" rel="noopener noreferrer" title="Partilhar no WhatsApp" class="share-btn whatsapp"><i class="fab fa-whatsapp"></i></a><a href="https://www.facebook.com/sharer/sharer.php?u=${o}" target="_blank" rel="noopener noreferrer" title="Partilhar no Facebook" class="share-btn facebook"><i class="fab fa-facebook-f"></i></a></div>`;
  }
  o(
    "/_dados/doacoes.json",
    "announcements-grid",
      ...
      return `
        <div class="col-md-4 mb-4">
            <div class="card h-100">
                ${
                  t.imagem
                    ? `<img src="${t.imagem}" class="card-img-top" alt="${t.titulo}">`
                    : `<div class="image-placeholder">${t.titulo}</div>`
                }
                <div class="card-body">
    ...
    o(
      "/_dados/servicos.json",
      "services-grid",
      function (t, e) {
        const a = t.logo_empresa
          ? `<div class="service-card-logo"><img src="${t.logo_empresa}" alt="Logo"></div>`
          : "";
        return `
        <div class="col-md-4 mb-4">
            <div class="card h-100">
                ${
                  t.valor_servico
                    ? `<div class="card-price">${t.valor_servico}</div>`
                    : ""
                }
                <div class="card-body">
                    ${a}
    ...
    o(
      ...
      function (t, e) {
        ...
        <div class="col-md-4 mb-4">
            <div class="card h-100">
                ${
                  t.imagens && t.imagens.length > 0 && t.imagens[0].imagem_url
                    ? `<img src="${t.imagens[0].imagem_url}" class="card-img-top" alt="${t.titulo}">`
                    : `<div class="image-placeholder">${t.titulo}</div>`
                }
                ${
                  t.valor_anuncio
                    ? `<div class="card-price">${t.valor_anuncio}</div>`
                    : ""
                }
                <div class="card-body">
                    <h5 class="card-title">${t.titulo}</h5>
    ...
    o(
      ...
      function (t) {
        ...
        return `
            <div class="col-lg-4 col-md-6 mb-4 blog-post-item" data-category="${
              t.category
            }">
    ...
    (async function () {
      const t = await n("/_dados/doacoes.json"),
        ...
        s = e.length,
        c = o + s + a.length;
      (document.getElementById("contador-doacoes").textContent = `${o}+`),
        (document.getElementById("contador-empregos").textContent = `${s}+`),
        (document.getElementById("contador-total").textContent = `${c}+`);
    })(),
      ...
      (async function () {
        ...
        t &&
          ((document.getElementById("hero-title").textContent =
            t.hero_title || "Bem-vindo ao Portugal Apoia"),
          (document.getElementById("hero-subtitle").textContent =
            t.hero_subtitle ||
            "A sua plataforma de apoio comunitário para encontrar e oferecer ajuda."),
          (document.getElementById("doacoes-text").textContent =
            t.impact_counters.doacoes_text || "Doações Realizadas"),
          (document.getElementById("empregos-text").textContent =
            t.impact_counters.emprego_text || "Empregos Publicados"),
          (document.getElementById("total-text").textContent =
            t.impact_counters.total_text || "Total de Anúncios"));
      })(),
    (function () {
